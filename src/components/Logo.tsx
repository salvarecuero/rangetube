...
export function Logo({ dark = false }: LogoProps) {
  ...
  return (
    <span className="inline-flex select-none flex-col gap-1.5" aria-label="RangeTube">
      <span
        aria-hidden="true"
        className={`font-logo text-[2.3rem] leading-none tracking-[-0.02em] ${dark ? "text-focus-ink" : "text-ink"}`}
      >
      ...
      <span aria-hidden="true" className="relative block h-3 w-full">
        <span
          className="absolute left-[9%] right-[9%] top-1/2 h-[4px] -translate-y-1/2 rounded-full"
          style={{ background: "linear-gradient(100deg,#0d9488,#2aa58a,#e9c46a 70%,#e76f51)" }}
        />
        <span
          className="absolute left-[9%] top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full"
          style={{ background: handleBg, boxShadow: "0 0 0 3px #0d9488" }}
        />
        <span
          className="absolute left-[91%] top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full"
          style={{ background: handleBg, boxShadow: "0 0 0 3px #d85a3d" }}
        />
      </span>
    </span>
  );
